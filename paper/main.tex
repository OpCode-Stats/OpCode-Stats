...
study of program instruction sequences.  Across two corpora totalling up to
3.74~million instructions, we characterise opcode frequency distributions,
n-gram entropy rates, compressibility, recurring motifs, and manifold
dimensionality, and validate that the measured statistics are stable as corpus
size grows.  The toolkit and all experimental data are publicly available.
\end{abstract}
...
utilities bzip2--xz with NCD $0.956$) are consistently closer than cross-category
...
x86-64 utilities spanning 2.91 million instructions, the toolkit shows that
opcode frequencies are heavily concentrated (MLE $\hat{\alpha} \approx 1.4$;
KS $=0.117$, indicating approximately but not strictly Zipfian), entropy rates decrease
by $1.12$ bits from $n=1$ to $n=5$ (vs.\ $0.30$ bits for a shuffled baseline,
...
clone detection recovers 537 clone pairs forming 117 families ($5.8\%$ of
functions) with a $96.6\%$ confirmation rate, revealing that the manifold is
