...
exhibit rich statistical structure that shares several properties observed
...
\begin{enumerate}
  ...
  \item \textbf{LZ78 complexity}~\cite{ziv1978compression}: the number of
        ...
        normalised statistic $\tilde{c} = K / \log_2 N$ (phrase count per
        log-bit of sequence length).  For high-entropy sources, the expected
        phrase count scales as $K = \Theta(N/\log N)$, so $\tilde{c}$ is
        correspondingly large for random sequences at $N \sim 10^6$.  Highly
        structured (more compressible) sequences yield substantially smaller
        $\tilde{c}$.
\end{enumerate}
...
vs.\ random $0.560$.  LZ78 normalised complexity ($\tilde{c} = K/\log_2 N$) has a median of $630$
phrases-per-log-bit for real binaries vs.\ $45{,}187$ for unigram-shuffled
sequences and $87{,}881$ for random sequences.
The large random-baseline magnitude ($\tilde{c}\!=\!87{,}881$) reflects
near-maximum-entropy behaviour; real instruction streams ($\tilde{c}\!=\!630$)
are roughly $140\times$ below that value and the unigram-shuffled baseline
($\tilde{c}\!=\!45{,}187$) sits between, confirming strong redundancy beyond
unigram skew.  All $\tilde{c}$ values are relative to the random baseline;
the key result is the ordering Real $\ll$ Shuffled $\ll$ Random.
